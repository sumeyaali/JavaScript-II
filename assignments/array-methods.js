...
const ticketPriceTotal = runners.reduce(function(accumulator, currentValue) {
  console.log(accumulator + currentValue.donation);
...
runners.forEach(function(items){
  return companyName.push(`${items.company_name}`);
})
...
let donations = runners.filter (function(items) {
  return (items.donation < 50) ;
});
...
const lastName = runners.map(function(currentValue){
